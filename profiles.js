...
exports.profile_manifests = {
    mojang: {
	  ...
	  const item = new profile_template()
                ...
                xml_parser.parseString(body, (inner_err, result) => {
                    ...
                    for (const index in packs) {
                        ...
                        for (const idx in old_versions) {
                            const new_item = new profile_template()
        ...
        handler: function(profile_dir, body, callback) {
            ...
            try {
                for (const index in body.promos) {
                    const item = new profile_template()
                    const ref_obj = body.promos[index]
                    ...
                    const ver = ref_obj['mcversion'].match(/(\d+)\.(\d+)\.?(\d+)?/)

                    if (parseInt(ver[1]) >= 1 && parseInt(ver[2]) >= 6) {
                        p.push(item)
                    } // 1.x major, .6 minor, chosen because 1.6.1 changed the installation process so drastically
                }
